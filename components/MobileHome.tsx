        ...
        (async () => {
            try {
                ...
                const { data } = await supabase
                    .from('pokemon_cards')
                    .select('id, name, english_name, set_id, number, rarity, image_small, image_large, language, raw_data->tcgplayer, pokemon_sets(name, printed_total, total), market_values(condition, language, market_avg, currency, last_updated)')
                    .eq('id', cardId)
                    .maybeSingle();
            ...
            appUrlListener = await App.addListener('appUrlOpen', async (data) => {
                ...
                try {
                    ...
                    if (appLink.protocol.startsWith('http') && appLink.pathname.startsWith('/live/')) {
                        // The table cam is included: the WebView CAN grant
