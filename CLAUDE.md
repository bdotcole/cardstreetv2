...
## Live multistream (social simulcast)

A live break is pushed to the seller's social channels (Facebook Live, YouTube, Instagram Live Producer, TikTok LIVE, Twitch, custom RTMP) by the **same LiveKit room-composite egress that records the VOD** — the destinations are added as RTMP stream outputs at go-live (`lib/livekit.ts:startRoomEgress`), so N channels cost the same egress-minutes as recording alone. Instead of LiveKit's built-in grid the egress renders **our template page `app/live/overlay`** (LiveKit custom-template contract: `?url&token&layout` query params, `START_RECORDING` / `END_RECORDING` console signals): the two feeds in the broadcaster's framing plus a burned-in CTA — `cardstreet.app/watch` as text + QR, the lot on the block (price, spots left), the pinned message and the chat tail. That overlay IS the funnel: a Facebook viewer has no buy button, so the frame tells them where it is.

- **`/watch`** (`app/watch/route.ts`) 302s to the public show that is live now (else the next scheduled, else `/live`), keeping any incoming UTMs and defaulting to `utm_source=social&utm_medium=overlay`. Ads and captions point here, never at a `/live/<uuid>`.
- **Destinations** live in `stream_destinations` (per seller; keys **AES-256-GCM encrypted** via `lib/streamKeyCrypto.ts`, key derived from `LIVE_DESTINATION_SECRET` or, by default, `SUPABASE_SERVICE_ROLE_KEY`; only a hint is ever returned). Per-show outcome per channel in `stream_simulcast_targets`, matched to LiveKit's `streamResults` by SHA-256 of the full URL — the full RTMP URL (it carries the key) is never logged and never stored. Both tables are RLS-on with **no policies** (service-role routes only).
- Routes: `app/api/live/destinations` (+ `[id]`) CRUD; `app/api/live/streams/[id]/simulcast` GET = per-channel status refreshed from LiveKit (`listEgress`), POST = mid-show add/remove via `updateStream` (starts a fresh egress if the old one is gone). The LiveKit webhook folds `egress_updated` / `egress_ended` results in too.
- Console UI: `components/live/MultistreamPanel.tsx` (above the Music panel). Canvas orientation (portrait 1080x1920 default / landscape) + overlay on/off are per-device localStorage prefs sent in the go-live body.
- Migration `20260916_stream_destinations.sql` — everything fails soft until it runs (the panel says so; go-live records the VOD only).
- **Hidden until show day:** `HIDDEN_ENTRY_POINTS` in `lib/betaFeatures.ts` lists `live_streams`, which removes the mobile Shop chooser door and the desktop nav "Live" item for everyone (admins included) while every /live URL, `/watch`, the console and the APIs keep working by link. It is deliberately weaker than `DISABLED_FEATURES` (which refuses the server gates too). Show day = delete the entry and push. Client surfaces read it via `isEntryPointHidden()` beside `hasBeta()`; `lib/betaAuth.ts` never consults it.
- Funnel analytics: `lib/liveEvents.ts` fires GA4 `live_view` / `live_spot_purchase` and Meta `ViewContent` / `Purchase` (THB) from the viewer, so live-stream ads can optimize on buyers.
- **Show audio** (`components/live/MusicPanel.tsx`): ONE WebAudio graph, ONE published track, two optional sources — the `live-music` bucket and `getDisplayMedia` desktop/tab capture — each with its own gain. The room permits a single extra audio publication (`publishExtraAudio` stops any previous one), which is why they are mixed rather than published separately. The capture branch connects to the destination **only, never `ctx.destination`**: the operator already hears that sound, and monitoring system audio feeds it back into the next capture buffer. Capture is desktop-browser only and feature-detects itself out of the app WebView, phones and Safari. YouTube Music cannot be integrated at all — no audio API, and its IFrame player is a cross-origin frame Web Audio cannot read; the panel carries a copyright warning because Facebook/TikTok fingerprint live audio.
- Known limits: social comments are NOT mirrored into CardStreet chat; one canvas orientation for all channels; Instagram/TikTok keys are per-broadcast; the overlay template needs a public https origin (local dev falls back to the grid layout). Runbook: `docs/live-multistream.md`.
...
- **Canonicals are locale-aware.** `buildAlternates(path, pathLocale)` makes each locale variant self-canonical, driven by the `x-cs-path-locale` header middleware derives from the **URL prefix only** — never the `cs_lang` cookie, or a returning English-preference visitor would flip the Thai canonical. Every hreflang alternate must be self-canonical; the previous always-Thai canonical de-indexed the whole `/en` tree. Noindexed pages (`/sell`, `/orders`, `/collection`, `/settings`) emit no hreflang, and `/help` is the one deliberate cross-canonical (→ `/faq`, whose FAQ accordion it duplicates), so it also carries no hreflang and is absent from the sitemap.
- **IndexNow** (`lib/indexNow.ts`) pings Bing/Yandex/Naver/Seznam — not Google — when a card page gains a listing, so pages jump the crawl queue instead of waiting behind ~80k discovered-but-unindexed URLs. Hooked into all three publish paths: `app/api/listings` POST, `app/api/alerts/listing-created` (the client-insert choke point), and `lib/draftListings.ts` (draft→active). **The key constant in `lib/indexNow.ts` and the filename in `public/<key>.txt` must stay identical** — a mismatch silently rejects every submission. Submissions are skipped off production so preview/dev never advertise unreachable URLs. Fails soft everywhere; indexing must never break a listing write.
