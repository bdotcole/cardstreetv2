...
export async function GET(request: Request, props: { params: Promise<{ setId: string }> }) {
    try {
        ...
        const { data: cards, error } = await supabase
            .from('pokemon_cards')
            .select('id, name, english_name, set_id, number, rarity, game, image_small, image_large, language, tcgplayer_url, raw_data->tcgplayer, pokemon_sets(name, printed_total, total), market_values(condition, language, market_avg, currency, last_updated)')
            .eq('set_id', setId)
