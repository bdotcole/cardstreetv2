...
export async function GET(request: Request, props: { params: Promise<{ setId: string }> }) {
    try {
        ...
        const { data: cards, error } = await supabase
            .from('pokemon_cards')
            .select('id, name, english_name, set_id, number, rarity, image_small, image_large, language, tcgplayer_url, raw_data->tcgplayer, pokemon_sets(name), market_values(market_avg, last_updated)')
            .ilike('set_id', setId)
