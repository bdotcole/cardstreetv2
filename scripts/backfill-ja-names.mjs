...
async function applyFixes(game, fixes) {
  ...
  if (EMIT_SQL) {
    const file = path.join(OUT_DIR, `ja-names-${game}.sql`);
    const lines = [
      `-- Japanese name backfill for ${game} (language='ja') -- ${fixes.length} rows`,
      `-- Generated by scripts/backfill-ja-names.mjs. Safe to re-run.`,
      `-- Rollback: english_name is preserved, so the prior value is recoverable per row.`,
      'BEGIN;',
    ];
    for (const r of rowsToWrite) {
      const sets = [`name = '${sqlEsc(r.ja)}'`];
      if (r.keepEnglish) sets.push(`english_name = COALESCE(english_name, '${sqlEsc(r.keepEnglish)}')`);
      lines.push(`UPDATE pokemon_cards SET ${sets.join(', ')} WHERE id = '${sqlEsc(r.id)}';`);
    }
    lines.push('COMMIT;');
    fs.writeFileSync(file, lines.join('\n'), 'utf-8');
    console.log(`  SQL written -> ${path.relative(ROOT, file)}`);
  }
